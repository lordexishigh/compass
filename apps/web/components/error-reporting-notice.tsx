...
import { readErrorReportingConsent } from '../lib/error-reporting-consent';
 ...
 *
 * ## And only when the answer was actually read
 *
 * `readErrorReportingConsent` rather than `errorReportingConsent`, because the two differ on
 * exactly the case that matters here. The flattened helper maps a failed read to `unset`, which is
 * the correct *gate* decision — do not send — but the wrong *sentence*: this notice sits in the
 * root layout, so a Postgres outage would put "nobody has decided yet" at the top of every page in
 * the product, including for an organization that answered long ago. Telling somebody their
 * recorded decision has evaporated, on the strength of a read that failed, is a claim the page
 * cannot support.
 *
 * A failed read renders nothing. That is not hiding the outage — `/api/health` reports the
 * database's condition, and this component's job is to carry one question, not to be a status
 * banner. The reader loses a prompt they may have already answered; they are told nothing false.
 */
export async function ErrorReportingNotice() {
  if (!errorReportingConfigured()) return null;

  const consent = await readErrorReportingConsent();
  if (!consent.ok) return null;
  if (consent.consent !== 'unset') return null;
