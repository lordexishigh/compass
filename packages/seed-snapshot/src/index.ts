...
/**
 * @compass/seed-snapshot — the seeded organization, projected into an
 * `AnalysisSnapshot` in memory, for any `(team, instant, window)`.
 *
 * ## Why this is a package rather than a test helper
 *
 * It began as `packages/analysis/tests/helpers/seed-snapshot.ts`, and for a long
 * time one consumer was all it needed. Two more arrived at once: the golden
 * fixture suite generates ten consecutive simulated days per team from it, and the
 * determinism gate re-generates the same day in a separate process. A helper
 * reachable only by a relative path into another package's `tests/` directory
 * cannot serve those without an import that the architecture rules exist to
 * forbid, so it is a package with an export instead. The old path still re-exports
 * this module, so every existing analysis test keeps its import unchanged.
 *
 * ## What it is a double for, stated plainly
 *
 * It does what ingest plus the snapshot builder do, in memory, with the same field
 * names and the same natural keys — `packages/ingest/src/naming.ts` is the
 * reference for both. It is a *test double for two layers*, and
 * `packages/pipeline/tests/contract.test.ts` is what proves the real snapshot
 * builder produces this shape. Golden fixtures generated from here are therefore
 * fixtures of the *analysis core over the seeded dataset*, which is exactly the
 * thing the acceptance bar is about; the pipeline's own persistence path has its
 * own tests.
 *
 * ## Why it may read files and use a clock when analysis may not
 *
 * The purity gate is about `packages/analysis/src`. This package is not the
 * analysis core: it is the fixture that feeds it, and building a fixture from the
 * checked-in seed means reading the seed. It takes `instant` as a parameter and
 * never asks a clock for `now`, so what it produces is a pure function of its
 * arguments — which is the property the golden fixtures depend on.
 */
